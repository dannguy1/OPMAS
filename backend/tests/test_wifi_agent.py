...
def test_compile_rule_patterns(wifi_agent):
    ...
    wifi_agent.agent_rules = {
        ...
        "DriverCrash": {"enabled": True, "crash_patterns": [r"(\w+): firmware crashed"]},
    }
...
async def test_check_deauth_flood(wifi_agent, deauth_log_event):
    ...
    wifi_agent.deauth_timestamps[client_mac] = deque(
        [current_time - 5, current_time - 2]  # 5 seconds ago  # 2 seconds ago
    )
...
async def test_check_dfs_radar(wifi_agent, dfs_log_event):
    ...
    wifi_agent.dfs_event_timestamps[event_key] = deque(
        [current_time - 1800, current_time - 900]  # 30 minutes ago  # 15 minutes ago
    )
...
async def test_process_log_event(
    wifi_agent, wifi_log_event, deauth_log_event, dfs_log_event, signal_log_event, crash_log_event
):
    ...
    assert (
        wifi_agent.nats_client.publish.call_count >= 0
