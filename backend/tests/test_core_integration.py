...
async def test_database_connection(db_session):
    ...
    # Create test device
    device = Device(
        hostname="test-router", ip_address="192.168.1.1", device_type="router", status="active"
    )
...
async def test_integrated_workflow(db_session, nats_client, redis_client):
    ...
    device = Device(
        hostname="test-device", ip_address="192.168.1.2", device_type="sensor", status="active"
    )
    db_session.add(device)
    db_session.commit()
    ...
    redis_client.hset(
        device_key, mapping={"status": "active", "last_seen": datetime.utcnow().isoformat()}
    )
