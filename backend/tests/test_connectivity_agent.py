...
def test_compile_rule_patterns(connectivity_agent):
    ...
    connectivity_agent.agent_rules = {
        ...
        "DNSResolution": {"enabled": True, "dns_patterns": [r"Failed to resolve domain (\S+)"]},
        "RoutingIssues": {"enabled": True, "routing_patterns": [r"No route to host (\S+)"]},
        "ServiceAvailability": {
            "enabled": True,
...
async def test_check_connection_timeouts(connectivity_agent, timeout_log_event):
    ...
    connectivity_agent.timeout_timestamps[timeout_key] = deque(
        [current_time - 30, current_time - 20]  # 30 seconds ago  # 20 seconds ago
    )
...
async def test_process_log_event(
    ...
    assert (
        connectivity_agent.nats_client.publish.call_count >= 0
