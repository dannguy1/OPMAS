...
async def test_complete_device_management_scenario(
    ...
    monitoring_messages = [
        msg for subject, msg in mock_nats.messages if subject == "devices.monitoring"
    ]
...
@pytest.mark.asyncio
async def test_error_handling_scenario(test_db, sample_device, mock_nats, mock_redis):
...
@pytest.mark.asyncio
async def test_performance_scenario(test_db, sample_devices, sample_logs, mock_nats, mock_redis):
