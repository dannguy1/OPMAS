...
async def test_device_discovery_workflow(test_db, mock_nats):
    ...
    discovery_messages = [msg for subject, msg in mock_nats.messages 
                         if subject == "devices.discovery"]
...
async def test_log_processing_workflow(test_db, sample_logs, mock_nats):
    ...
    notification_messages = [msg for subject, msg in mock_nats.messages 
                           if subject == "logs.processed"]
...
async def test_device_monitoring_workflow(test_db, sample_device, mock_nats):
    ...
    # Verify NATS notifications
    status_messages = [msg for subject, msg in mock_nats.messages 
                      if subject == "devices.status"]
