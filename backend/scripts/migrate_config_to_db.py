...
try:
    from src.opmas.config import load_config, get_config # Load bootstrap config
    from src.opmas.db_utils import get_db_session, init_db
    from src.opmas.db_models import (
        OpmasConfig, Agent, AgentRule, Playbook, PlaybookStep
    )
...
def migrate_core_config(session, yaml_data):
    ...
    if not yaml_data or 'logging' not in yaml_data:
        logger.warning("Core config YAML missing or lacks 'logging' section. Cannot migrate settings.")
        return
...
def migrate_agents_and_rules(session, yaml_data):
    ...
    if not yaml_data or 'agent_rules' not in yaml_data:
        logger.warning("Agent rules YAML missing or lacks 'agent_rules' section. Skipping rule migration FROM YAML.")
        return
    ...
    for agent_name, rules in rules_by_agent.items():
        ...
        if not agent:
             logger.warning(f"Agent '{agent_name}' found in YAML but not in DB. Skipping rule migration for this agent. Ensure agents are created first.")
             continue
        ...
        for rule_dict in rules:
            if not isinstance(rule_dict, dict):
                logger.warning(f"Invalid rule format for agent '{agent_name}' (not a dict): {rule_dict}. Skipping.")
                continue

            rule_name = rule_dict.get('name')
            if not rule_name:
                logger.warning(f"Rule for agent '{agent_name}' missing 'name': {rule_dict}. Skipping.")
                continue

            # Rule config is the entire dictionary for that rule
            rule_config_json = rule_dict

            existing_rule = session.query(AgentRule).filter_by(agent_id=agent.agent_id, rule_name=rule_name).first()
            if existing_rule:
                if json.dumps(existing_rule.rule_config, sort_keys=True) != json.dumps(rule_config_json, sort_keys=True):
                    logger.info(f"Updating rule '{rule_name}' for agent '{agent_name}'")
                    existing_rule.rule_config = rule_config_json
                else:
                    logger.info(f"Rule '{rule_name}' for agent '{agent_name}' already exists and matches. Skipping.")
            else:
...
def migrate_playbooks(session, yaml_data):
    ...
    if not yaml_data or 'action_playbooks' not in yaml_data:
        logger.warning("Playbook YAML missing or lacks 'action_playbooks' section. Skipping playbook migration.")
        return
    ...
    for finding_type, steps in playbooks_dict.items():
        try:
            ...
            if not playbook:
                ...
                logger.info(f"Playbook for '{finding_type}' already exists (ID: {playbook.playbook_id}). Updating steps.")
                # Delete existing steps for simplicity, then re-add. More robust merge is complex.
                logger.warning(f"Deleting existing steps for playbook '{finding_type}' before re-adding.")
                session.query(PlaybookStep).filter_by(playbook_id=playbook.playbook_id).delete()
                session.flush()

            # --- Migrate Steps ---
            if not isinstance(steps, list):
                logger.warning(f"Steps for playbook '{finding_type}' are not a list. Skipping steps.")
                continue

            for i, step_dict in enumerate(steps):
                if not isinstance(step_dict, dict):
                    logger.warning(f"Invalid step format for playbook '{finding_type}' (not a dict): {step_dict}. Skipping.")
                    continue

                action_type = step_dict.get('action_type')
                if not action_type:
                    logger.warning(f"Step {i} for playbook '{finding_type}' missing 'action_type'. Skipping.")
                    continue
...
def ensure_core_agents_exist(session):
    ...
    for agent_data in core_agents:
        try:
            ...
            logger.error(f"Error ensuring core agent '{agent_data.get('name', 'UNKNOWN')}': {e}", exc_info=True)
            session.rollback()
...
def main():
    ...
    try:
        with get_db_session() as session:
            ...
            migrate_agents_and_rules(session, agent_rules_data) # Now only adds/updates rules if agent exists
