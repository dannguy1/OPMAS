...
def send_batch(api_url: str, batch: List[str], source_id: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 202:
            logger.debug(f"Successfully sent batch of {len(batch)} lines. Response: {response.json()}")
            return True
        else:
            # This case might not be reached often due to raise_for_status, but good practice
            logger.warning(f"API returned unexpected status code {response.status_code}. Response: {response.text}")
            return False
        ...
        logger.error(f"HTTP error sending batch to {api_url}: {e.response.status_code} - {e.response.text}")
        return False
...
def main():
    ...
    try:
        from src.opmas.config import load_config, get_config
    except ImportError:
        pass

    parser = argparse.ArgumentParser(description="Read a log file and send its contents in batches to the OPMAS Log API.")
    parser.add_argument("logfile", help="Path to the raw log file.")
    parser.add_argument("--api-base-url", default="http://localhost:8000", help="Base URL of the OPMAS Log API (default: http://localhost:8000)")
    parser.add_argument("--source-id", default=None, help="Optional source identifier to send with the logs.")
    parser.add_argument("--batch-size", type=int, default=100, help="Number of log lines per API request batch (default: 100)")
    parser.add_argument("--delay", type=float, default=0, help="Optional delay (in seconds) between sending batches (default: 0)")
    ...
    logger.info(f"Batch size: {batch_size}, Source ID: '{args.source_id or 'Not Set'}', Delay: {args.delay}s")
    ...
    try:
        with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            if current_batch:
                logger.info(f"Sending final batch {batches_sent + 1} ({len(current_batch)} lines)...")
                success = send_batch(api_endpoint, current_batch, args.source_id)
                if success:
                    batches_sent += 1
                    total_lines_sent += len(current_batch)
