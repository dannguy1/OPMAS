...
class AgentUpdate(BaseModel):
    ...
    subscribed_topics: Optional[List[str]] = Field(None, description="List of NATS topics to subscribe to")
...
class AgentRuleUpdate(BaseModel):
    ...
    severity: Optional[FindingSeverity] = Field(None, description="Severity level for findings from this rule")
    enabled: Optional[bool] = Field(None, description="Whether the rule is enabled")
    cooldown_seconds: Optional[int] = Field(None, description="Cooldown period between findings in seconds")
    threshold: Optional[int] = Field(None, description="Number of occurrences required to trigger a finding")
