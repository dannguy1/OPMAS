...
try:
    ...
    from .mq import publish_message, get_shared_nats_client # Need get_shared_nats_client for startup
    from .parsing_utils import parse_syslog_line, classify_nats_subject, DEFAULT_SUBJECT # Import DEFAULT_SUBJECT
...
class LogIngestionRequest(BaseModel):
    ...
    source_identifier: Optional[str] = Field(None, description="Optional identifier for the log source (e.g., hostname or IP).")
    explicit_source_ip: Optional[str] = Field(None, description="Optional: Explicit IP address of the original log source device.")

# --- Background Task for Processing ---
async def process_and_publish_logs(log_lines: List[str], source_identifier: Optional[str], client_ip: Optional[str], explicit_source_ip: Optional[str]):
    ...
    for line in log_lines:
        try:
            ...
            if not parsed_data:
                ...
                logger.debug(f"Failed to parse log line via syslog regex (assuming generic): {line[:100]}...")
    ...
    if publish_tasks:
        ...
        logger.info(f"Background processing complete for {len(log_lines)} lines. Events published: {success_count}, Failures: {failure_count}.")
    else:
...
async def ingest_logs(request: Request, payload: LogIngestionRequest, background_tasks: BackgroundTasks):
    """
    ...
    if not payload.logs:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No logs provided in the request body.")

    num_logs = len(payload.logs)
    source_id = payload.source_identifier or 'unspecified'
    logger.info(f"Received API request to ingest {num_logs} log lines from '{source_id}' (Client: {client_ip}, Explicit Source: {explicit_ip or 'None'}).")

    # Add the processing task to run in the background
    background_tasks.add_task(process_and_publish_logs, payload.logs, payload.source_identifier, client_ip, explicit_ip)

    return {"message": f"Log ingestion request for {num_logs} lines accepted and queued for background processing."}
