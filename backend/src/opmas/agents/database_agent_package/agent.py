...
class DatabaseAgent(BaseAgent):
    ...
    def _initialize_state(self):
        ...
        self.connection_timestamps: Dict[str, Deque[Tuple[str, float]]] = defaultdict(lambda: deque())
        ...
        self.integrity_timestamps: Dict[str, Deque[Tuple[str, float]]] = defaultdict(lambda: deque())
...
async def main():
    ...
    if DATABASE_AGENT_PID_FILE.is_file():
        print(f"ERROR: PID file {DATABASE_AGENT_PID_FILE} already exists. Is another DatabaseAgent process running? Exiting.", flush=True)
        exit(1)
    ...
    try:
        ...
        print(f"DatabaseAgent started with PID {os.getpid()}. PID file: {DATABASE_AGENT_PID_FILE}", flush=True)
    except IOError as e:
        print(f"ERROR: Failed to write PID file {DATABASE_AGENT_PID_FILE}: {e}. Exiting.", flush=True)
        exit(1)
