...
class ApplicationAgent(BaseAgent):
    ...
    def _initialize_state(self):
        ...
        self.performance_timestamps: Dict[str, Deque[Tuple[str, float]]] = defaultdict(
            lambda: deque()
        )
...
async def main():
    ...
    if APPLICATION_AGENT_PID_FILE.is_file():
        print(
            f"ERROR: PID file {APPLICATION_AGENT_PID_FILE} already exists. Is another ApplicationAgent process running? Exiting.",
            flush=True,
        )
        exit(1)
    ...
    try:
        ...
        print(
            f"ApplicationAgent started with PID {os.getpid()}. PID file: {APPLICATION_AGENT_PID_FILE}",
            flush=True,
        )
    except IOError as e:
        print(
            f"ERROR: Failed to write PID file {APPLICATION_AGENT_PID_FILE}: {e}. Exiting.",
            flush=True,
        )
        exit(1)
