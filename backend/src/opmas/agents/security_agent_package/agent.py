...
async def main():
    ...
    if SECURITY_AGENT_PID_FILE.is_file():
        print(
            f"ERROR: PID file {SECURITY_AGENT_PID_FILE} already exists. Is another SecurityAgent process running? Exiting.",
            flush=True,
        )
        exit(1)
    ...
    try:
        ...
        print(
            f"SecurityAgent started with PID {os.getpid()}. PID file: {SECURITY_AGENT_PID_FILE}",
            flush=True,
        )
    except IOError as e:
        print(
            f"ERROR: Failed to write PID file {SECURITY_AGENT_PID_FILE}: {e}. Exiting.", flush=True
        )
        exit(1)
