...
async def main():
    ...
    if STORAGE_AGENT_PID_FILE.is_file():
        print(
            f"ERROR: PID file {STORAGE_AGENT_PID_FILE} already exists. Is another StorageAgent process running? Exiting.",
            flush=True,
        )
        exit(1)
    ...
    try:
        ...
        print(
            f"StorageAgent started with PID {os.getpid()}. PID file: {STORAGE_AGENT_PID_FILE}",
            flush=True,
        )
    except IOError as e:
        print(
            f"ERROR: Failed to write PID file {STORAGE_AGENT_PID_FILE}: {e}. Exiting.", flush=True
        )
        exit(1)
