...
class BaseAgent(ABC):
    ...
    def __init__(self, agent_name: str, subscribed_topics: List[str], findings_topic: str, queue_group: str = "", load_rules_from_config: bool = True):
        ...
        if load_rules_from_config:
            ...
            self._load_env_rules()
             ...
             self.logger.info("Skipping default rule loading based on 'load_rules_from_config=False'. Subclass should load rules.")
    ...
    def _find_package_env(self) -> Optional[Path]:
        ...
        try:
            ...
            if env_file.exists():
                ...
                self.logger.warning(f"Found example.env but no .env in {agent_dir}. Using example.env.")
                return example_env
    ...
    def _load_agent_rules(self) -> Dict[str, Any]:
        ...
        if not rules_path.is_absolute():
            ...
            project_root = Path(__file__).resolve().parents[4]  # Go up one more level to project root
    ...
    def _load_env_rules(self):
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(patterns)} default classification rules from package environment")
            ...
            self.logger.error(f"Error loading default rules from package environment: {e}", exc_info=True)
        ...
        for rule_name, rule_config in self.agent_rules.items():
            ...
            for pattern in patterns:
                if pattern.lower() in event.message.lower():
                    ...
                    event.structured_fields['classifications'] = event.structured_fields.get('classifications', [])
                    event.structured_fields['classifications'].append({
                        'rule_name': rule_name,
                        'pattern': pattern,
                        'timestamp': datetime.now().isoformat()
                    })
                    self.logger.info(f"Event {event.event_id} matched classification rule '{rule_name}' with pattern '{pattern}'")
    ...
    async def _message_handler(self, msg):
        ...
        # +++ Log Entry & Raw Data +++
        self.logger.debug(f"[{self.agent_name}] _message_handler received message on '{subject}'. Raw data: {data_str[:150]}...")
        # ++++++++++++++++++++++++++++
        try:
            ...
            self.logger.debug(f"[{self.agent_name}] Instantiated ParsedLogEvent: ID={log_event.event_id}, Type={log_event.log_source_type}, Msg='{log_event.message[:50]}...'")
            # ++++++++++++++++++++++++++++++
            await self.process_log_event(log_event)
        except json.JSONDecodeError:
            self.logger.error(f"Failed to decode JSON from '{subject}': {data_str[:100]}...")
        except TypeError as e:
            self.logger.error(f"Failed to instantiate ParsedLogEvent from '{subject}' data: {e}. Data: {data_dict}")
        except Exception as e:
    ...
    async def subscribe(self):
        ...
        for sub in self.subscriptions:
             try: await sub.unsubscribe()
                  ...
                  self.logger.warning(f"[{self.agent_name}] Error unsubscribing previous subscription: {e}")
        self.subscriptions = []
        ...
        for topic in self.subscribed_topics:
            ...
            try:
                ...
                self.logger.info(f"[{self.agent_name}] Successfully subscribed to topic: '{topic}' with queue '{queue_group}'")
                # ++++++++++++++++++++++++++++++++++++
            except Exception as e:
                 # +++ Add Subscription Failure Log +++
                 self.logger.error(f"[{self.agent_name}] Failed to subscribe to topic '{topic}': {e}", exc_info=True)
                 # ++++++++++++++++++++++++++++++++++++
    ...
    async def run(self):
        ...
        try:
            ...
            print(f"CRITICAL ERROR getting logger for {getattr(self, 'agent_name', 'UnknownAgent')}: {log_init_e}", flush=True)
        ...
        try:
            ...
            agent_logger.critical(f"Error during initial startup of {self.agent_name}: {start_e}", exc_info=True)
            self.running = False
        ...
        try: # <<< Main try block >>>
            ...
            agent_logger.info(f"NATS connection attempt complete for {self.agent_name}. Connected: {connection_status}")
            ...
            agent_logger.info(f"{self.agent_name} started successfully. Waiting for events or shutdown signal.")
            await self.shutdown_event.wait()

        except Exception as e: # <<< Except for main try block >>>
            agent_logger.critical(f"{self.agent_name} encountered a critical error during run: {e}", exc_info=True)
