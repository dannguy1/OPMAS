...
class ConnectivityAgent(BaseAgent):
    ...
    def _initialize_state(self):
        ...
        self.interface_status_timestamps: Dict[str, Deque[Tuple[str, float]]] = defaultdict(
            lambda: deque()
        )
        ...
        self.service_status_timestamps: Dict[str, Deque[Tuple[str, float]]] = defaultdict(
            lambda: deque()
        )
...
async def main():
    ...
    if CONNECTIVITY_AGENT_PID_FILE.is_file():
        print(
            f"ERROR: PID file {CONNECTIVITY_AGENT_PID_FILE} already exists. Is another ConnectivityAgent process running? Exiting.",
            flush=True,
        )
        exit(1)
    ...
    try:
        ...
        print(
            f"ConnectivityAgent started with PID {os.getpid()}. PID file: {CONNECTIVITY_AGENT_PID_FILE}",
            flush=True,
        )
    except IOError as e:
        print(
            f"ERROR: Failed to write PID file {CONNECTIVITY_AGENT_PID_FILE}: {e}. Exiting.",
            flush=True,
        )
        exit(1)
