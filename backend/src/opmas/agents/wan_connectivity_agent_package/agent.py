...
class WANConnectivityAgent(BaseAgent):
    ...
    def _initialize_state(self):
        ...
        self.interface_event_timestamps: Dict[Tuple[str, str], Deque[float]] = defaultdict(
            lambda: deque()
        )
            ...
            if isinstance(rule_config, dict) and rule_config.get("enabled", False):
                ...
                for pattern_str in down_patterns_cfg:
                    try:
                        ...
                        self.logger.error(
                            f"Failed to compile down_pattern for rule '{rule_name}': '{pattern_str}'. Error: {e}"
                        )
                ...
                for pattern_str in up_patterns_cfg:
                    try:
                        ...
                        self.logger.error(
                            f"Failed to compile up_pattern for rule '{rule_name}': '{pattern_str}'. Error: {e}"
                        )

                if compiled_down or compiled_up:
                    self.compiled_patterns[rule_name] = {"down": compiled_down, "up": compiled_up}
                    self.logger.debug(
                        f"Compiled {len(compiled_down)} down / {len(compiled_up)} up patterns for rule '{rule_name}'"
                    )
    ...
    async def _check_interface_flapping(self, event: ParsedLogEvent):
        ...
        self.logger.debug(
            f"_check_interface_flapping called for event_id: {event.event_id}, message: '{event.message}'"
        )
        # -----------------------------
        ...
        for pattern in down_patterns:
            ...
            self.logger.debug(
                f"Checking down_pattern '{pattern.pattern}' against message: '{event.message}'"
            )
            # -----------------------------------------
            ...
            if match:
                ...
                try:
                    ...
                    interface_name = (
                        match.group(1) if len(match.groups()) >= 1 else "unknown_interface"
                    )
                    ...
                    self.logger.warning(
                        f"Regex down_pattern '{pattern.pattern}' matched but has no capture group for interface name."
                    )
                    interface_name = "unknown_interface"  # Set a default
                    break
                except Exception as e:
                    self.logger.error(
                        f"Error extracting interface from down_pattern '{pattern.pattern}': {e}"
                    )
        ...
        if not event_type:
            for pattern in up_patterns:
                # --- Add Debug Log For Pattern Matching ---
                self.logger.debug(
                    f"Checking up_pattern '{pattern.pattern}' against message: '{event.message}'"
                )
                # -----------------------------------------
                match = pattern.search(event.message)
                if match:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Regex up_pattern '{pattern.pattern}' matched but has no capture group for interface name."
                        )
                        interface_name = "unknown_interface"
                        break
                    except Exception as e:
                        self.logger.error(
                            f"Error extracting interface from up_pattern '{pattern.pattern}': {e}"
                        )
                        interface_name = "unknown_interface"
                        break
        ...
        if interface_name and event_type:
            ...
            self.logger.debug(
                f"Interface event '{event_type}' detected for {interface_name} on {device_key}"
            )
            ...
            self.logger.debug(
                f"Interface event count check for {interface_name} on {device_key}: Count={event_count}, Threshold={flap_threshold}"
            )
            # -------------------------------------
            ...
            if event_count >= flap_threshold:
                ...
                if current_time - last_finding_time > finding_cooldown_seconds:
                    self.logger.warning(
                        f"Interface flapping detected for {interface_name} on {device_key} ({event_count} events in {time_window_seconds}s)"
                    )
                    ...
                    self.logger.debug(
                        f"Attempting to publish finding: {finding.finding_type} ({finding.finding_id}) to {self.findings_topic}"
                    )
                    # -------------------------------------
                    ...
                    self.logger.debug(
                        f"Interface flapping for {interface_name} on {device_key} still active but within cooldown period."
                    )
