...
async def main():
    ...
    if SYSTEM_AGENT_PID_FILE.is_file():
        print(
            f"ERROR: PID file {SYSTEM_AGENT_PID_FILE} already exists. Is another SystemAgent process running? Exiting.",
            flush=True,
        )
        exit(1)
    ...
    try:
        ...
        print(
            f"SystemAgent started with PID {os.getpid()}. PID file: {SYSTEM_AGENT_PID_FILE}",
            flush=True,
        )
    except IOError as e:
