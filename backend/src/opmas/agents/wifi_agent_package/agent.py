...
class WiFiAgent(BaseAgent):
    ...
    def _load_rules_from_db(self):
        ...
        try:
            with get_db_session() as session:
                ...
                db_rules = (
                    session.query(AgentRule).filter_by(agent_id=agent_id, is_enabled=True).all()
                )
                ...
                for rule in db_rules:
                    if rule.rule_config and isinstance(rule.rule_config, dict):
                        ...
                        loaded_count += 1
                        self.logger.debug(
                            f"Loaded enabled rule '{rule.rule_name}' (ID: {rule.rule_id})"
                        )
                    else:
                        self.logger.warning(
                            f"Rule '{rule.rule_name}' (ID: {rule.rule_id}) for agent {agent_id} is enabled but has invalid/missing config. Skipping."
                        )

                self.agent_rules = temp_rules  # Assign loaded rules
                self.logger.info(
                    f"Successfully loaded {loaded_count} enabled rules for '{self.agent_name}'."
                )
                self._initialize_state()

        except SQLAlchemyError as e:
            self.logger.critical(
                f"Database error loading rules for '{self.agent_name}': {e}", exc_info=True
            )
            self.agent_rules = {}
        except Exception as e:
            self.logger.critical(
                f"Unexpected error loading rules from DB for '{self.agent_name}': {e}",
                exc_info=True,
            )
            self.agent_rules = {}
    ...
    def _initialize_state(self):
        ...
        self.signal_strength_readings: Dict[str, Deque[Tuple[float, float]]] = defaultdict(
            lambda: deque()
        )
    ...
    async def _check_high_auth_failure(self, event: ParsedLogEvent):
            ...
            if match:
                try:
                    ...
                    if re.match(r"^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$", mac_from_match):
                        ...
                        self.logger.debug(
                            f"Rule '{rule_name}' matched failure pattern. MAC: {client_mac}"
                        )
                        break  # Found MAC, process this event
                    else:
                        self.logger.warning(
                            f"Rule '{rule_name}' matched pattern but group 1 ('{mac_from_match}') is not a valid MAC address."
                        )
                except IndexError:
                    self.logger.warning(
                        f"Rule '{rule_name}' matched pattern '{pattern.pattern}' but regex has no capture group for MAC address."
                    )
                except Exception as e:
        ...
        if failure_count >= failure_threshold:
            self.logger.warning(
                f"High auth failure rate detected for MAC {client_mac} ({failure_count} failures in {time_window_seconds}s)"
            )
            ...
            self.logger.debug(
                f"Auth failure count for {client_mac} is {failure_count}, below threshold {failure_threshold}."
            )
...
async def main():
    ...
    if WIFI_AGENT_PID_FILE.is_file():
        print(
            f"ERROR: PID file {WIFI_AGENT_PID_FILE} already exists. Is another WiFiAgent process running? Exiting.",
            flush=True,
        )
        exit(1)
    ...
    try:
        ...
        print(
            f"WiFiAgent started with PID {os.getpid()}. PID file: {WIFI_AGENT_PID_FILE}", flush=True
        )
    except IOError as e:
