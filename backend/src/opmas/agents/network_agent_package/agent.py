...
class NetworkAgent(BaseAgent):
    ...
    def _initialize_state(self):
        ...
        self.connectivity_timestamps: Dict[str, Deque[Tuple[str, float]]] = defaultdict(
            lambda: deque()
        )
        self.recent_connectivity_findings: Dict[str, float] = {}

        # State for BandwidthIssues rule
        self.bandwidth_timestamps: Dict[str, Deque[Tuple[str, float]]] = defaultdict(
            lambda: deque()
        )
...
async def main():
    ...
    if NETWORK_AGENT_PID_FILE.is_file():
        print(
            f"ERROR: PID file {NETWORK_AGENT_PID_FILE} already exists. Is another NetworkAgent process running? Exiting.",
            flush=True,
        )
        exit(1)
    ...
    try:
        ...
        print(
            f"NetworkAgent started with PID {os.getpid()}. PID file: {NETWORK_AGENT_PID_FILE}",
            flush=True,
        )
    except IOError as e:
        print(
            f"ERROR: Failed to write PID file {NETWORK_AGENT_PID_FILE}: {e}. Exiting.", flush=True
        )
        exit(1)
