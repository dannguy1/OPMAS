...
class PerformanceAgent(BaseAgent):
    ...
    def _initialize_state(self):
        ...
        self.cpu_usage_timestamps: Dict[str, Deque[Tuple[float, float]]] = defaultdict(
            lambda: deque()
        )
        self.recent_cpu_findings: Dict[str, float] = {}

        # State for MemoryPressure rule
        self.memory_pressure_timestamps: Dict[str, Deque[Tuple[float, float]]] = defaultdict(
            lambda: deque()
        )
        self.recent_memory_findings: Dict[str, float] = {}

        # State for DiskIO rule
        self.disk_io_timestamps: Dict[str, Deque[Tuple[str, float, float]]] = defaultdict(
            lambda: deque()
        )
        self.recent_disk_findings: Dict[str, float] = {}

        # State for ProcessResources rule
        self.process_resource_timestamps: Dict[str, Deque[Tuple[str, float, float]]] = defaultdict(
            lambda: deque()
        )
        self.recent_process_findings: Dict[str, float] = {}

        # State for SystemLoad rule
        self.system_load_timestamps: Dict[str, Deque[Tuple[float, float]]] = defaultdict(
            lambda: deque()
        )
        ...
        for pattern in patterns:
            ...
            if match:
                try:
                    ...
                    self.logger.warning(
                        f"Error extracting process resource details from pattern: {e}"
                    )
                    continue
...
async def main():
    ...
    if PERFORMANCE_AGENT_PID_FILE.is_file():
        print(
            f"ERROR: PID file {PERFORMANCE_AGENT_PID_FILE} already exists. Is another PerformanceAgent process running? Exiting.",
            flush=True,
        )
        exit(1)
    ...
    try:
        ...
        print(
            f"PerformanceAgent started with PID {os.getpid()}. PID file: {PERFORMANCE_AGENT_PID_FILE}",
            flush=True,
        )
    except IOError as e:
        print(
            f"ERROR: Failed to write PID file {PERFORMANCE_AGENT_PID_FILE}: {e}. Exiting.",
            flush=True,
        )
        exit(1)
