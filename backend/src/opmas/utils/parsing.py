...
def classify_log_source(process_name: str, message: str) -> str:
    ...
    if process_name == "kernel":
        if any(
            ...
            for keyword in ["oom-killer", "jffs2", "ubifs", "error", "warning"]
        ):
            return "health"
        if any(
            keyword in message.lower()
            for keyword in ["ath10k", "mt76", "wlan", "wifi", "ieee80211"]
        ):
            return "wifi"
...
async def process_raw_log(log_data: dict):
    ...
    source_identifier = log_data.get(
    ...
    if not event.log_source_type:
        logger.debug(
            f"Log event from {event.hostname or event.source_ip} could not be classified. Discarding. Message: {event.message[:100]}"
        )
    ...
    while True:
        ...
        try:
            ...
            if nats_topic and event_dict:
                ...
                logger.debug(
                    f"Attempting publish to {raw_topic} for event {event_dict['event_id']}"
                )
                # ++++++++++++++++++++++++++++
                ...
                if event_dict["log_source_type"]:
                    ...
                    logger.debug(
                        f"Attempting publish to {specific_topic} for event {event_dict['event_id']}"
                    )
                    # +++++++++++++++++++++++++++++++++
                    ...
                    logger.warning(
                        f"Log event {event_dict['event_id']} could not be classified, only published to raw topic."
                    )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async def simulate_ingestor():
            ...
            test_logs = [
                {
                    "raw_message": "<30>Apr 18 15:02:17 OpenWRT-Router1 hostapd[1234]: wlan0: STA aa:bb:cc:dd:ee:ff IEEE 802.11: authenticated",
                    "source_ip": "192.168.1.1",
                    "arrival_ts_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
                },
                {
                    "raw_message": "<13>Apr 18 15:02:18 OpenWRT-Router1 kernel: [ 123.456] device eth0 entered promiscuous mode",
                    "source_ip": "192.168.1.1",
                    "arrival_ts_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
                },
                {
                    "raw_message": "<86>Apr 18 15:02:19 OpenWRT-Router1 dnsmasq[5678]: DHCPACK(br-lan) 192.168.1.100 aa:bb:cc:dd:ee:ff test-client",
                    "source_ip": "192.168.1.1",
                    "arrival_ts_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
                },
                {
                    "raw_message": "<29>Apr 18 15:02:20 OpenWRT-Router1 dropbear[9101]: Bad password attempt for root from 192.168.1.50:12345",
                    "source_ip": "192.168.1.1",
                    "arrival_ts_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
                },
                {
