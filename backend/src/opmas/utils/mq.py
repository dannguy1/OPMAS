...
async def publish_message(
    subject: str, message_dict: dict, nats_client: Optional[nats.NATS] = None
):
...
async def subscribe_handler(subject: str, callback: callable, nats_client=None, queue=""):
    ...
    try:
        ...
        async def message_handler(msg):
            ...
            logger.debug(
                f"Received message on subject '{msg_subject}': {data_str[:100]}..."
            )  # Log truncated data
            try:
                ...
                logger.error(
                    f"Failed to decode JSON from message on subject '{msg_subject}': {data_str}"
                )
            except Exception as e:
                logger.error(
                    f"Error processing message from subject '{msg_subject}': {e}", exc_info=True
                )
        ...
        logger.error(
            f"An error occurred during NATS subscription setup for '{subject}': {e}", exc_info=True
        )
        if should_close_on_error and nc and nc.is_connected:
            await nc.close()  # Clean up the client we created
        return None, None
