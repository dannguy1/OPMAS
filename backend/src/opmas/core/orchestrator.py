...
from .data_models import ActionResult, AgentFinding
from .db_models import Agent as AgentModel
from .db_models import Finding as FindingModel  # Alias DB models
from .db_models import IntendedAction as IntendedActionModel
from .db_models import Playbook, PlaybookStep
...
class OrchestratorAgent:
    ...
    def __init__(self):
        ...
        logger.debug(
            f"[{self.AGENT_NAME}] Playbooks loaded from DB. Keys: {list(self.playbooks.keys()) if isinstance(self.playbooks, dict) else 'N/A'}"
        )
        ...
        try:
            with get_db_session() as session:
                ...
                for pb in db_playbooks:
                    ...
                    for (
                        step
                    ) in pb.steps:  # Steps are ordered by step_order due to relationship config
                        steps_list.append(
                            {
                    ...
                    logger.debug(
                        f"Loaded playbook '{pb.name}' for finding type '{pb.finding_type}' with {len(steps_list)} steps."
                    )

            logger.info(
                f"Successfully loaded {len(playbooks_structured)} playbooks from the database."
            )
    ...
    async def _handle_finding(self, msg: nats.aio.msg.Msg):
        ...
        try:
            ...
            try:
                with get_db_session() as session:
                    ...
                    if agent:
                        ...
                        logger.warning(
                            f"Agent '{finding_obj.agent_name}' not found in DB. Cannot link finding."
                        )
                ...
                logger.info(
                    f"Stored finding '{finding_obj.finding_type}' ({finding_obj.finding_id}) to database."
                )
            except SQLAlchemyError as db_e:
                logger.error(
                    f"Database error storing finding {finding_obj.finding_id}: {db_e}",
                    exc_info=True,
                )
                ...
                logger.error(
                    f"Unexpected error storing finding {finding_obj.finding_id}: {e}", exc_info=True
                )
                # Continue?
            # --------------------------------

            logger.info(
                f"Processing finding '{finding_obj.finding_type}' ({finding_obj.finding_id}) from {finding_obj.agent_name} for {finding_obj.device_hostname or finding_obj.device_ip}"
            )
            ...
            logger.error(
                f"Failed to instantiate AgentFinding Pydantic model from '{subject}' data: {e}. Data: {finding_data}"
            )
        except Exception as e:
    ...
    def _render_command(self, template_str: str, finding: AgentFinding) -> Optional[str]:
        ...
        try:
            ...
            if "{{ " in rendered_command or " }} " in rendered_command:
                logger.warning(
                    f"Command template '{template_str}' might have unresolved variables: '{rendered_command}'"
                )
    ...
    async def _execute_playbook(self, finding: AgentFinding):
        ...
        if not device_key:
            logger.error(
                f"Finding {finding.finding_id} lacks device identifier. Cannot process playbook."
            )
            return
        ...
        if not playbook_steps:
            ...
            logger.info(
                f"No specific playbook found for finding type '{finding_type}'. No action taken."
            )
            return
        ...
        for i, step in enumerate(playbook_steps):
            ...
            if current_time - last_run_time < cooldown_seconds:
                logger.debug(
                    f"Intended action '{action_type_name}' for device {device_key} (step {i}) is on cooldown. Skipping logging."
                )
                continue
            # -----------------------------------------------------
            ...
            if command_template:
                ...
                if rendered_command is None:
                    logger.error(
                        f"Step {i}: Command rendering failed for playbook '{finding_type}'. Logging action without command context."
                    )
            ...
            logger.info(
                f"Playbook '{finding_type}' triggered for finding {finding.finding_id}. Logging intended action."
            )
            try:
                with get_db_session() as session:
                ...
                logger.info(
                    f"Stored intended action '{action_type_name}' for finding {finding.finding_id} to database."
                )

            except SQLAlchemyError as db_e:
                logger.error(
                    f"Database error storing intended action for finding {finding.finding_id}: {db_e}",
                    exc_info=True,
                )
                ...
                logger.error(
                    f"Unexpected error storing intended action for finding {finding.finding_id}: {e}",
                    exc_info=True,
                )
                break
            # --------------------------------------
            ...
            logger.info(
                f"Completed first action processing (step {i}) for playbook '{finding_type}'. Stopping playbook execution."
            )
...
async def main():
    ...
    try:
        ...
        logger.critical(
            f"Failed to load configuration, setup logging, or initialize DB: {e}", exc_info=True
        )
    ...
    if initial_setup_ok:
        try:
            ...
            print(
                f"CRITICAL ERROR: Failed to create PID file {ORCHESTRATOR_PID_FILE}. Exiting.",
                file=sys.stderr,
            )
