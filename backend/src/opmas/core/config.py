...
def load_config(config_path: Path = CORE_CONFIG_FILE_PATH) -> None:
    ...
    if loaded_data is None:
        logger.critical(
            f"Failed to load core configuration from {config_path_str}. OPMAS cannot start correctly."
        )
        _config = None
...
def _create_default_core_config(
    default_config: Dict[str, Any], file_path: Path = CORE_CONFIG_FILE_PATH
):
    ...
    if not file_path.exists():
        try:
            ...
            logger.error(
                f"Failed to create default core config file {file_path}: {e}", exc_info=True
            )
