    ...
    def datagram_received(self, data: bytes, addr: Tuple[str, int]):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Decoding error with UTF-8 from {source_ip}:{source_port}, trying latin-1...")
            ...
            try:
                ...
                logger.warning(f"Internal raw log queue is full! Discarding message from {source_ip}. Consider increasing queue size or parser performance.")

        except Exception as e:
            logger.error(f"Error processing datagram from {source_ip}:{source_port}: {e}", exc_info=True)
...
async def start_log_ingestor(log_queue: asyncio.Queue):
    ...
    try:
        ...
        await asyncio.Event().wait() # Keep running indefinitely until interrupted
         ...
         logger.error(f"Permission denied to bind to port {udp_port}. Try running as root or using a port > 1024.")
    except OSError as e:
