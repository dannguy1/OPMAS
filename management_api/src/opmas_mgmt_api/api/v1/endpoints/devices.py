...
async def create_device(
    device: DeviceCreate, db: AsyncSession = Depends(get_db), nats: NATSManager = Depends(get_nats)
) -> DeviceResponse:
    """Create a new device."""
    service = DeviceService(db, nats)
    try:
...
async def get_device(
    device_id: UUID, db: AsyncSession = Depends(get_db), nats: NATSManager = Depends(get_nats)
) -> DeviceResponse:
    """Get device by ID."""
    service = DeviceService(db, nats)
    try:
