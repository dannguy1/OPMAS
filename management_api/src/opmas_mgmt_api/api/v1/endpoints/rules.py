...
from opmas_mgmt_api.schemas.rules import RuleCreate, RuleList, RuleResponse, RuleStatus, RuleUpdate
...
async def create_rule(
    rule: RuleCreate, db: AsyncSession = Depends(get_db), nats: NATSManager = Depends(get_nats)
) -> RuleResponse:
    """Create a new rule."""
    service = RuleService(db, nats)
    try:
...
async def get_rule(
    rule_id: UUID, db: AsyncSession = Depends(get_db), nats: NATSManager = Depends(get_nats)
) -> RuleResponse:
    """Get rule by ID."""
    service = RuleService(db, nats)
    try:
