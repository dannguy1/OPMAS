...
class PlaybookCreate(BaseModel):
    name: str
    finding_type: str = Field(..., description="The finding type that triggers this playbook (must be unique)")
    description: Optional[str] = None
...
async def list_playbooks(
    ...
    playbooks = query.order_by(playbook_models.Playbook.created_at.desc()).offset(skip).limit(limit).all()
    return playbooks
...
async def create_playbook(
    ...
    db_playbook = db.query(playbook_models.Playbook).filter(
        playbook_models.Playbook.name == playbook.name
    ).first()
    if db_playbook:
        raise HTTPException(
            status_code=400,
...
async def get_playbook(
    ...
    playbook = db.query(playbook_models.Playbook).filter(
        playbook_models.Playbook.id == playbook_id
    ).first()
    if playbook is None:
...
async def update_playbook(
    ...
    if playbook.name and playbook.name != db_playbook.name:
        existing = db.query(playbook_models.Playbook).filter(
            playbook_models.Playbook.name == playbook.name
        ).first()
        if existing:
            raise HTTPException(
...
@router.post("/playbooks/{playbook_id}/execute", response_model=playbook_schemas.PlaybookExecutionInDB)
async def execute_playbook(
    ...
    playbook = db.query(playbook_models.Playbook).filter(
        playbook_models.Playbook.id == playbook_id,
        playbook_models.Playbook.is_active == True
    ).first()
    if playbook is None:
        raise HTTPException(
            status_code=404,
    ...
    execution = execution_models.PlaybookExecution(
        ...
        status=execution_models.ExecutionStatus.PENDING
    )
    db.add(execution)
    db.commit()
...
@router.get("/playbooks/{playbook_id}/executions", response_model=List[playbook_schemas.PlaybookExecutionInDB])
async def list_playbook_executions(
    ...
    executions = query.order_by(execution_models.PlaybookExecution.created_at.desc()).offset(skip).limit(limit).all()
    return executions

@router.get("/playbooks/executions/{execution_id}", response_model=playbook_schemas.PlaybookExecutionInDB)
async def get_execution(
    execution_id: int,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_active_user)
):
    execution = db.query(execution_models.PlaybookExecution).filter(
        execution_models.PlaybookExecution.id == execution_id
    ).first()
    if execution is None:
        raise HTTPException(status_code=404, detail="Execution not found")
    return execution

@router.put("/playbooks/executions/{execution_id}", response_model=playbook_schemas.PlaybookExecutionInDB)
...
async def delete_playbook_step(
    ...
    try:
        step_orm = db.query(PlaybookStepModel).filter(
            PlaybookStepModel.playbook_id == playbook_id,
            PlaybookStepModel.step_id == step_id
        ).first()
        if step_orm is None:
            logger.warning(f"Step with ID {step_id} not found in playbook {playbook_id} for deletion.")
            playbook_exists = db.query(PlaybookModel.playbook_id).filter(PlaybookModel.playbook_id == playbook_id).first()
            if not playbook_exists:
                 raise HTTPException(status_code=404, detail=f"Playbook with ID {playbook_id} not found")
            else:
                 raise HTTPException(status_code=404, detail=f"Step with ID {step_id} not found in playbook {playbook_id}")
        db.delete(step_orm)
        db.commit()
        ...
        db.rollback() 
        logger.error(f"Error deleting step {step_id} from playbook {playbook_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to delete step {step_id} from playbook {playbook_id}")
# --------------------------
...
async def add_playbook_step(
    ...
    logger.info(f"Received request to ADD step to playbook {playbook_id} with data: {step_data.dict()}")
    ...
    if not playbook:
        ...
        raise HTTPException(status_code=404, detail=f"Playbook with ID {playbook_id} not found")
    try:
        max_order = db.query(func.max(PlaybookStepModel.step_order)).filter(
            PlaybookStepModel.playbook_id == playbook_id
        ).scalar()
        ...
        db.commit()
        db.refresh(new_step)
        logger.info(f"Successfully added step {new_step.step_id} (order {new_step.step_order}) to playbook {playbook_id}")
        return new_step
...
async def update_playbook_step(
    ...
    logger.info(f"Received request to UPDATE step {step_id} in playbook {playbook_id} with data: {step_update_data.dict()}")
    step_orm = db.query(PlaybookStepModel).filter(
        PlaybookStepModel.playbook_id == playbook_id,
        PlaybookStepModel.step_id == step_id
    ).first()
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Database error updating step {step_id} in playbook {playbook_id}: {db_e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Database error updating step: {db_e}")
    except Exception as e:
        db.rollback()
        logger.error(f"Unexpected error updating step {step_id} in playbook {playbook_id}: {e}", exc_info=True)
