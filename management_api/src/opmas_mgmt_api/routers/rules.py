...
async def create_rule(
    ...
    db_rule = db.query(rule_models.Rule).filter(rule_models.Rule.name == rule.name).first()
    if db_rule:
        raise HTTPException(status_code=400, detail="Rule with this name already exists")

    # Check if agent exists
    agent = db.query(agent_models.Agent).filter(agent_models.Agent.id == rule.agent_id).first()
    if not agent:
...
async def get_rule(
    rule_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)
):
    rule = db.query(rule_models.Rule).filter(rule_models.Rule.id == rule_id).first()
    if rule is None:
...
async def update_rule(
    ...
    db_rule = db.query(rule_models.Rule).filter(rule_models.Rule.id == rule_id).first()
    if db_rule is None:
        raise HTTPException(status_code=404, detail="Rule not found")

    # Check name uniqueness if being updated
    if rule.name and rule.name != db_rule.name:
        existing = db.query(rule_models.Rule).filter(rule_models.Rule.name == rule.name).first()
        if existing:
            raise HTTPException(status_code=400, detail="Rule with this name already exists")
...
async def get_rule_status(
    ...
    return {"enabled": rule.enabled, "updated_at": rule.updated_at or rule.created_at}
