...
async def create_agent(agent_data: AgentCreate, db: Session = Depends(get_db)) -> AgentResponse:
    ...
    if not agent_package_full_path.is_dir():
        ...
        raise HTTPException(
            status_code=400, detail=f"Agent package path '{agent_data.package_path}' not found."
        )
    try:
        ...
        logger.info(
            f"Successfully created agent config '{new_agent_orm.name}' with ID: {new_agent_orm.agent_id}"
        )
...
@router.get("/", response_model=AgentsApiResponse, summary="Get a list of configured agents")
async def list_agents(
    ...
    logger.info(
        f"Received request for GET /agents (skip={skip}, limit={limit}, device_id={device_id}, status={status}, enabled={enabled})"
    )
    ...
    return AgentsApiResponse(agents=mapped_agents, total=total, limit=limit, offset=skip)
            ...
            if item.is_dir() and item.name != "__pycache__" and not item.name.endswith("_template"):
                ...
                if env_file.is_file() and agent_py_file.is_file():
                    ...
                    try:
                        ...
                        if not agent_name:
                            logger.warning(
                                f"Skipping package '{package_name}': AGENT_NAME missing in .env file."
                            )
                            continue
                        if agent_name in existing_agent_names:
                            logger.debug(
                                f"Skipping already configured agent (by name): '{agent_name}' from package '{package_name}'"
                            )
                            continue
                        logger.info(
                            f"Discovered potential new agent: name='{agent_name}', description='{agent_desc}', package='{package_name}'"
                        )
                        discovered.append(
                            DiscoveredAgent(
                                name=agent_name, package_path=package_name, description=agent_desc
                            )
                        )
                    except Exception as env_err:
                        logger.error(
                            f"Error processing .env file for package '{package_name}': {env_err}",
                            exc_info=True,
                        )
                        continue
    except Exception as e:
...
async def update_agent(
    agent_id: int, agent_update_data: AgentUpdate, db: Session = Depends(get_db)
) -> AgentResponse:
    logger.info(
        f"Received request to UPDATE agent ID {agent_id} with data: {agent_update_data.dict()}"
    )
...
async def list_agent_rules(
    agent_id: int, limit: int = 50, offset: int = 0, db: Session = Depends(get_db)
    ...
    try:
        ...
        return AgentRulesApiResponse(rules=rules_orm, total=total, limit=limit, offset=offset)
    except Exception as e:
        logger.error(f"Error fetching rules for agent {agent_id} from database: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch rules for agent {agent_id} from database"
        )
...
async def create_agent_rule(
    agent_id: int, rule_data: AgentRuleCreate, db: Session = Depends(get_db)
) -> AgentRuleResponse:
    """Creates a new rule for the specified agent."""
    logger.info(
        f"Received request to CREATE rule for agent {agent_id} with data: {rule_data.dict()}"
    )
    ...
    existing_rule = (
        db.query(AgentRuleModel)
        .filter(
            AgentRuleModel.agent_id == agent_id, AgentRuleModel.rule_name == rule_data.rule_name
        )
        .first()
    )
    if existing_rule:
        logger.warning(
            f"Rule with name '{rule_data.rule_name}' already exists for agent {agent_id}"
        )
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
    ...
    try:
        new_rule = AgentRuleModel(
            agent_id=agent_id, rule_name=rule_data.rule_name, rule_config=rule_data.rule_config
        )
        db.add(new_rule)
        db.commit()
        db.refresh(new_rule)
...
async def update_agent_rule(
    agent_id: int, rule_id: int, rule_update_data: AgentRuleUpdate, db: Session = Depends(get_db)
) -> AgentRuleResponse:
    """Updates an existing rule for the specified agent."""
    logger.info(f"Received request to UPDATE rule {rule_id} for agent {agent_id}")
    rule_orm = (
        db.query(AgentRuleModel)
        .filter(AgentRuleModel.agent_id == agent_id, AgentRuleModel.rule_id == rule_id)
        .first()
    )
    ...
    if rule_orm is None:
        ...
        if not agent_exists:
            raise HTTPException(status_code=404, detail=f"Agent with ID {agent_id} not found")
        raise HTTPException(
            status_code=404, detail=f"Rule with ID {rule_id} not found for agent {agent_id}"
        )
    ...
    if rule_update_data.rule_name != rule_orm.rule_name:
        ...
        if name_conflict:
            logger.warning(
                f"Rule name '{rule_update_data.rule_name}' already exists for agent {agent_id}"
            )
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
    ...
    try:
        ...
        db.rollback()
        logger.error(
            f"Database error updating rule {rule_id} for agent {agent_id}: {db_e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail=f"Database error updating rule: {db_e}")
    except Exception as e:
        db.rollback()
        logger.error(
            f"Unexpected error updating rule {rule_id} for agent {agent_id}: {e}", exc_info=True
        )
...
async def delete_agent_rule(agent_id: int, rule_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.rollback()
        logger.error(
            f"Database error deleting rule {rule_id} from agent {agent_id}: {db_e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail=f"Database error deleting rule: {db_e}")
    except Exception as e:
        db.rollback()
        logger.error(
            f"Unexpected error deleting rule {rule_id} from agent {agent_id}: {e}", exc_info=True
        )
...
async def get_agent(
    agent_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)
):
    agent = db.query(agent_models.Agent).filter(agent_models.Agent.id == agent_id).first()
    if agent is None:
...
async def update_agent(
    ...
    db_agent = db.query(agent_models.Agent).filter(agent_models.Agent.id == agent_id).first()
    if db_agent is None:
        raise HTTPException(status_code=404, detail="Agent not found")
    ...
    if agent.name and agent.name != db_agent.name:
        existing = (
            db.query(agent_models.Agent).filter(agent_models.Agent.name == agent.name).first()
        )
        if existing:
    ...
    if agent.device_id and agent.device_id != db_agent.device_id:
        device = (
            ...
            .filter(device_models.Device.id == agent.device_id)
            .first()
        )
        if not device:
