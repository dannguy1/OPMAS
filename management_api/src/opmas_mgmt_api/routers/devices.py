...
async def create_device(
    ...
    db_device = (
        ...
        .filter(device_models.Device.hostname == device.hostname)
        .first()
    )
    if db_device:
    ...
    # Create new device
    db_device = device_models.Device(**device.dict(), status="pending", last_seen=datetime.utcnow())
    db.add(db_device)
    db.commit()
...
async def get_device(
    device_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_active_user)
):
    device = db.query(device_models.Device).filter(device_models.Device.id == device_id).first()
    if device is None:
...
async def update_device(
    ...
    db_device = db.query(device_models.Device).filter(device_models.Device.id == device_id).first()
...
async def get_device_status(
    ...
    return {"status": device.status, "last_seen": device.last_seen, "is_active": device.is_active}
