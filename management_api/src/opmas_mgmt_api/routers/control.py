...
from opmas_mgmt_api.schemas.control import ControlActionResponse, ControlActionStatus
...
async def _execute_control_script(
    action: str, component: Optional[str], control_service: ControlService, action_id: str
) -> None:
    """Execute control script and update action status."""
    try:
        # Update status to in progress
        await control_service.update_control_action(action_id, ControlActionStatus.IN_PROGRESS)
        ...
        # Execute command
        result = subprocess.run(cmd, capture_output=True, text=True, check=False, cwd=_PROJECT_ROOT)
        ...
        if result.returncode == 0:
            await control_service.update_control_action(
                ...
                {"stdout": result.stdout, "return_code": result.returncode},
            )
        ...
        await control_service.update_control_action(
            action_id, ControlActionStatus.FAILED, {"error": str(e)}
        )
...
async def start_system(
    ...
    if not START_SCRIPT_PATH.is_file() or not os.access(START_SCRIPT_PATH, os.X_OK):
        raise HTTPException(
            status_code=500, detail="System start script is missing or not executable."
        )
    ...
    background_tasks.add_task(
        _execute_control_script, "start", component, control_service, action.id
    )
...
async def stop_system(
    ...
    if not START_SCRIPT_PATH.is_file() or not os.access(START_SCRIPT_PATH, os.X_OK):
        raise HTTPException(
            status_code=500, detail="System control script is missing or not executable."
        )
    ...
    # Execute in background
    background_tasks.add_task(
        _execute_control_script, "stop", component, control_service, action.id
    )
...
async def restart_system(
    ...
    # Execute in background
    background_tasks.add_task(
        _execute_control_script, "restart", component, control_service, action.id
    )
...
async def reload_configuration(
    ...
    # Execute in background
    background_tasks.add_task(
        _execute_control_script, "reload", component, control_service, action.id
    )
...
async def get_control_status(
    action_id: str, db: AsyncSession = Depends(get_db), nats: NATSManager = Depends(get_nats)
) -> ControlActionResponse:
...
@router.get("/actions", response_model=List[ControlActionResponse], summary="List control actions")
...
@router.get(
    "/component/{component}/status", response_model=Dict[str, Any], summary="Get component status"
)
async def get_component_status(
    component: str, db: AsyncSession = Depends(get_db), nats: NATSManager = Depends(get_nats)
) -> Dict[str, Any]:
