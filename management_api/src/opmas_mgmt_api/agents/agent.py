...
class AgentService:
    ...
    async def get_agents(
        self, skip: int = 0, limit: int = 100, agent_type: Optional[str] = None
    ) -> List[Agent]:
        """Get list of agents."""
        query = select(Agent)
        if agent_type:
    ...
    async def delete_agent(self, agent_id: str) -> bool:
        ...
        await self.nats.publish(
            "agent.deleted", {"agent_id": agent_id, "timestamp": datetime.utcnow().isoformat()}
        )
    ...
    async def get_agent_rule(self, agent_id: str, rule_id: str) -> Optional[AgentRule]:
        ...
        query = select(AgentRule).where(AgentRule.agent_id == agent_id, AgentRule.id == rule_id)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()

    async def create_agent_rule(self, agent_id: str, rule: AgentRuleCreate) -> Optional[AgentRule]:
    ...
    async def update_agent_rule(
        self, agent_id: str, rule_id: str, rule: AgentRuleUpdate
    ) -> Optional[AgentRule]:
        """Update agent rule."""
    ...
    async def delete_agent_rule(self, agent_id: str, rule_id: str) -> bool:
        ...
        await self.nats.publish(
            "agent.rule.deleted",
            {"agent_id": agent_id, "rule_id": rule_id, "timestamp": datetime.utcnow().isoformat()},
        )
    ...
    async def update_agent_status(
        ...
        await self.nats.publish(
            ...
            {"agent_id": agent_id, "status": status, "timestamp": datetime.utcnow().isoformat()},
        )
